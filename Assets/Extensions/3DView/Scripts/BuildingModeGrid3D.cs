...
namespace CBSK
{
    public class BuildingModeGrid3D : BuildingModeGrid
    {
        ...
        override public Vector3 GridPositionToWorldPosition(GridPosition position, GridPosition[] shape)
        {
            ...
            return new Vector3(position.x * widthOfGridInWorldUnits, 0, position.y * heightOfGridInWorldUnits);

        }

        override public GridPosition WorldPositionToGridPosition(Vector3 position)
        {
            return new GridPosition((int)((position.x) / widthOfGridInWorldUnits), (int)((position.z) / heightOfGridInWorldUnits));
        }
