...
namespace CBSK
{
    public class PathManager : Manager<PathManager>
    {
        ...
        virtual public string GetSpriteSuffix(Building building)
        {
			...
			if (gridObject != null && gridObject is Building && ((Building)gridObject) != building &&  ((Building)gridObject).Type == building.Type) suffix += "N";

            gridObject = grid.GetObjectAtPosition(building.Position + new GridPosition(0, -1));
			if (gridObject != null && gridObject is Building && ((Building)gridObject) != building &&  ((Building)gridObject).Type == building.Type) suffix += "E";

            gridObject = grid.GetObjectAtPosition(building.Position + new GridPosition(-1, 0));
			if (gridObject != null && gridObject is Building && ((Building)gridObject) != building &&  ((Building)gridObject).Type == building.Type) suffix += "S";

            gridObject = grid.GetObjectAtPosition(building.Position + new GridPosition(0, 1));
			if (gridObject != null && gridObject is Building && ((Building)gridObject) != building &&  ((Building)gridObject).Type == building.Type) suffix += "W";
